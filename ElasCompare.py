...
class ElasCompare:
    ...
    def compare_index(self):
        for string, eid in self.elas_index_data.items():  # loop elas_index_data
            if string in self.filenames_list:  # loop filenames from logs
                lid = self.elasLogs.read_previous_logs(string)  # get data from logs
                if lid is None or "pri_store_size" not in lid:  # check pss and none
                    self.total_dps += dps_to_total_dps(eid['pri_store_size'])
                    self.elasLogs.add_to_logs(filename=string, source=eid, dps=eid['pri_store_size'])
                    ...
                    self.elasLogs.add_to_logs(filename=string, source=eid, dps=dps)
    ...
    def add_tdps(self):
        self.elasLogs.add_to_logs(filename='tdpss-', source="tdpss-", dps=self.total_dps)
